...
void loop() {
	...
	if (currentState == TURNED_OFF) {
		...
		if (game.getFuelRemaining() > 0.0 && (game.getNextFlare() != -1)) {
		...
		spinForMemoryPack(opponentMemoryPackPos);
		...
		game.getPOILoc(firstPOI, 0);
		stopAtFastest(firstPOI);
